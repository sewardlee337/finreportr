...
**finreportr** provides two functions that allows users to acquire background information relating to publicly-listed companies.

The function `CompanyInfo()` returns the following information about a given company based on its stock ticker symbol:
...
The function `AnnualReports()` returns a data frame that summarizes a given company's annual reports based on its stock ticker symbol. It will return data relating to a company's [Form 10-Ks](http://www.investopedia.com/terms/1/10-k.asp) if the company is domestic, and [Form 20-Fs](http://www.investopedia.com/terms/s/sec-form-20-f.asp) if the company is foreign. 
...
The main benefit of **finreportr** comes from allowing users to search and display financial data without having to manually inspect filings from the SEC EDGAR database. This opens up opportunities for users to introduce automation, scalability, and reproducibility to their analysis.
...
**finreportr** contains two datasets that serve as lookup tables for encoded data displayed by the function `CompanyInfo`.

* The dataset `siccodes` provides a listing of SIC codes and the industries that they represent.
* The dataset `statecodes` provides a listing of state and country codes used by the SEC EDGAR database, and the states or countries that they represent.